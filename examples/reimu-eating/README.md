...
- **Breathing** — a 1.5 px sine on the whole body, one cycle per loop.
...
- **Secondary sway** — a 0.5 px horizontal sine; deliberately restrained.
...
Track `target` values (`body`, `head`, `hand_right`, `eyes`, `mouth`) are
semantic labels for a future renderer; the harness does not claim a flattened
